...
char* indexes(const char* path)
{
    ...
    char* string1 = malloc(strlen(php) + strlen(path) + 1);
    if (string1 == NULL)
        return false; 
    char* string2 = malloc(strlen(html) + strlen(path) + 1);
    if (string2 == NULL)
        return false;
    strcpy(string1, path); //concatenate path string with index.php and index.html to check if it's in directory.
    strcat(string1, php);
    strcpy(string2, path);
    strcat(string2, html);
    if (access(string1, F_OK) == 0) //access returns 0 if successfully found in directory otherwise returns -1.
        return string1;
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // initialize content and other tracking variables to NULL, 
    *content = NULL;
    size_t len = 0;
    size_t bytes = 0;
    BYTE buffer[BYTES]; //allocate buffer of 512 bytes.
    do
    {     //loop through file stream.
        bytes = fread(&buffer, 1, BYTES, file); //fread returns size based on amt bytes read. Read in 512 byte blocks.
        *content = realloc(*content, len + bytes); //reallocate memory based on amount of bytes read through stream, len grows according to bytes successfully passed. 
        memcpy(*content + len, &buffer, bytes); //memcpy takes bytes size of info from buffer and stores it at stream pointed to by *content + len.
        len += bytes;                          //you add len to *content because you don't want to overwrite previously copied data.
    }                                     //increase len by bytes successfully copied over. Rmb that len represents bytes that stream has passed in.
    while (bytes == 512); 
    *length = len; //dereference length by total bytes copied (as is tracked by one of your tracking variables). 
...
const char* lookup(const char* path)
{
    ...
    char copy[length+1]; //include space for null terminator.
    strcpy(copy, path);
    char* fullStop = strrchr(copy, '.'); //searches for last instance of fullstop in path.
    if (strcasecmp(fullStop, ".css") == 0) //strcasecmp will return zero if strings compared in both arguments are of equal size. (case insensitive)
        return "text/css";
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char copyCat[linelen];
    strcpy(copyCat, line);
    int count = 0;
    for (int i = 0; i < linelen; i++) //loop searches for instances of spaces in line (request-line). Must be equal to 2.
    {
        if (copyCat[i] == ' ')
            count++;
    ...
    char* firstSpace = strchr(line, ' '); //this code breaks up line into method - request - http version 
    int methodSize = firstSpace - line;
    char method[methodSize];
    strncpy(method, line, methodSize);
    char* secondSpace = strrchr(line, ' ');
    int reqSize = secondSpace - firstSpace - 1; //sizes are decremented by one because the space pointers point to the beginning of the space.
    char req[reqSize];
    strncpy(req, firstSpace+1, reqSize); //as per above comment, you don't start copying the string till one place in mem past ptr to first space, or you copy the space.
    req[reqSize] = '\0';                     //its necessary to create char arrays so you can null terminate the strings passed to abs_path and query.
    ...
    if (qmark != NULL)
    {
        ...
        int absSize = qmark - firstSpace - 1;
        if (querySize > 0)
        {
            char queryCopy[querySize];
            strncpy(queryCopy, qmark+1, querySize);
            queryCopy[querySize] = '\0';
            strncpy(query, queryCopy, querySize+1); //you copy + 1 to include the null terminator.
            req[absSize] = '\0';
            strncpy(abs_path, req, absSize+1);
        }
        else if (querySize == 0) //a q mark could be present with nothing past it. 
        {
            req[absSize] = '\0';
            strncpy(abs_path, req, absSize+1);
            query = "";
